...
void 	(*INIT_BLOK)(void)	=InitADC9;
void 	(*DRIVER_BLOK)(void)=DriverADC9;
void	(*SERVICE_PAK_UART)(BYTE, BYTE*, WORD) = ServiceUart;

WORD (*STATE_BLOCK) = &Adc9.Info.word;
...
void InitADC9()
{
	...
	for(i=0; i<ADC_CHIP_COUNT; i++)
	{
		...
		{
			Adc9.adc_mux_set[i][j] = MUX_INPUT;
			Adc9.adc_mux_new[i][j] = MUX_INPUT;
		}
	}
	//--------------------------------------------------------
	for(i=0; i<ADC_CHIP_COUNT; i++)
	{
		for(j=0; j<12; j++)
		{
			Adc9.reg_adc_set[i][j] = RegDefADC[j];
			Adc9.reg_adc_new[i][j] = RegDefADC[j];
		}
	}
	//------------------------------------
	Adc9.Info.word = 0;
...
void ServiceUart(BYTE Id, BYTE* pData, WORD Len)
{
	...
	if(Id == 0x03)//Write
	{
		memmove(Adc9.RecvPak, &pData[1], pData[0]);
		Adc9.lenRecvPak = pData[0];
		Adc9.stRecvPak = TRUE;
		return;
	}
