...
; 3  History
;====================================================================
; Id: START.ASM 7268 2011-05-30 09:11:06Z rlande 

#define VERSION  "1.64"
/*
Revision 1.64 7268 2011-05-30 09:11:06Z rlande 
- corrected: bank register for CONST2 section

Revision 1.63 6929 2011-04-14 11:11:37Z rranze
- added: MB96355RxB
- added: new disclaimer

Revision 1.62 5575 2010-06-02 14:34:28Z rlande
- updated: Note on changing clock settings
- added: Check for CLKB maximum setting

Revision 1.61 5144 2010-04-12 11:49:30Z rlande
- updated: Disclaimer
- corrected: Clock setting protection for MB96388HxA

Revision 1.60 4104 2009-11-16 12:20:03Z rlande
- added: Clock setting CPU_48MHZ_CLKP1_24MHZ_CLKP2_16MHZ.
...
Revision 1.57 3734 2009-09-03 08:57:19Z rlande
- added: Check of clock settings for MB96384RxA, MB96384YxA

Revision 1.56 3700 2009-08-24 15:38:49Z rlande
...
Revision 1.55 3360 2009-07-24 12:29:46Z rlande
- removed: series MB96360

Revision 1.54 3179 2009-07-03 08:53:18Z rlande
...
#set      SERIES    MB96330        ; <<< select Series
...
#set      DEVICE    MB96338UxA      ; <<< select device
...
#set      CPU_48MHZ_CLKP1_24MHZ_CLKP2_16MHZ   0x30181030L
...
#set      CLOCK_SPEED     CPU_48MHZ_CLKP2_16MHZ  ; <<< set clock speeds
...
; wait state settings (execution speed) as well as the power consumption.
; 
; If "WAIT_STATE_OPTIMIZATION" is selected, a PLL setting with minimum
; flash wait states is applied. This can increase performance. Note that 
...
; Note: The settings for CRYSTAL, CLOCK_SPEED and CLOCK_OPTIMIZATION
; may affect the PLL configuration, system clocks, clock dividers,
; flash timing settings and background debugging settings of the MCU 
; to achieve the selected clock speeds.
; The clock control registers and flash timing registers (see MB96300_HWM)
; will most probably be set to values other than their default.
; 
; Attention: When changing clocks during application runtime (inside
; main() ) please take care for flash timing settings as well, as the
; flash timing settings generated in start.asm may have minimum 
; and/or maximum ranges for System Clock 1 (CLKS1) and Bus Clock
; (CLKB). Please also take care that background debugging may fail
; when the clock settings are altered during application runtime (inside
; main() ).
...
#set      VERSION_STAMP     ON     ; <<< enable version number in
                                    ; separated section
...
; file can be used for initialising all sections.
   ...
   .DATA.H CONST2,   BNKSEC CONST2,   CINIT2, BNKSEC CINIT2, SIZEOF CINIT2
#endif
...
#elif CLOCK_SPEED == CPU_48MHZ_CLKP1_24MHZ_CLKP2_16MHZ
#  set   CLK_PARAMS_4MHZ_PLL_LOW 0x0C12314   ; CLKS1=CLKS2=48MHZ, CLK_VCO=192MHZ
#  set   CLK_PARAMS_4MHZ_CLKB2   0x1824622   ; CLKS1=CLKS2=96MHZ, CLK_VCO=192MHZ
#  set   CLK_PARAMS_8MHZ_PLL_LOW 0x0612314   ; CLKS1=CLKS2=48MHZ, CLK_VCO=192MHZ
#  set   CLK_PARAMS_8MHZ_CLKB2   0x0C24622   ; CLKS1=CLKS2=96MHZ, CLK_VCO=192MHZ
#  set  CLK_PARAMS_16MHZ_PLL_LOW 0x0312314   ; CLKS1=CLKS2=48MHZ, CLK_VCO=192MHZ
#  set  CLK_PARAMS_16MHZ_CLKB2   0x0624622   ; CLKS1=CLKS2=96MHZ, CLK_VCO=192MHZ
...
#endif
#if ((CLKB(CLK_PARAMS) > (56 * FREQ_1MHZ))) || ((CLKP1(CLK_PARAMS) > (56 * FREQ_1MHZ)))
#  error The current clock setting ("CLOCK_SPEED") is not allowed for the selected device.
#endif
...
#if (SERIES == MB96330)
#  if ((CLKB(CLK_PARAMS) > (48 * FREQ_1MHZ))) || ((CLKP1(CLK_PARAMS) > (48 * FREQ_1MHZ)))
#    error The current clock setting ("CLOCK_SPEED") is not allowed for the selected device.
#  endif
...
#    error The current clock setting ("CLOCK_SPEED" and "CLOCK_OPTIMIZATION") is not allowed for the selected device.
#  endif
#  if ((CLKB(CLK_PARAMS) > (40 * FREQ_1MHZ))) || ((CLKP1(CLK_PARAMS) > (40 * FREQ_1MHZ)))
#    error The current clock setting ("CLOCK_SPEED") is not allowed for the selected device.
#  endif
...
#      error The current clock setting ("CLOCK_SPEED" and "CLOCK_OPTIMIZATION") is not allowed for the selected device.
#    endif
#    if ((CLKP1(CLK_PARAMS) > (40 * FREQ_1MHZ))) || ((CLKP1(CLK_PARAMS) > (40 * FREQ_1MHZ)))
#      error The current clock setting ("CLOCK_SPEED") is not allowed for the selected device.
#    endif
#  endif
